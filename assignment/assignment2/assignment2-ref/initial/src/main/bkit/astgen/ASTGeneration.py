...
class ASTGeneration(BKITVisitor):
    ...
    def visitFuncdecls(self, ctx:BKITParser.FuncdeclsContext):
        ...
        if ctx.parameterlist():
            return FuncDecl(Id(ctx.ID().getText()),ctx.parameterlist().accept(self),(reduce(lambda a,b: a + b, filter(lambda x: isinstance(x, list), statements), []),filter(lambda x: not(isinstance(x,list)), statements)))
        else:
            return FuncDecl(Id(ctx.ID().getText()),[],(reduce(lambda a,b: a + b, filter(lambda x: isinstance(x, list), statements), []),filter(lambda x: not(isinstance(x,list)), statements)))
    ...
    def visitIf_statement(self, ctx:BKITParser.If_statementContext):
        statements = list(map(lambda x: x.accept(self), ctx.statement()))
        return [reduce(lambda a,b: a + b, filter(lambda x: isinstance(x, list), statements), []),filter(lambda x: not(isinstance(x,list)), statements)]
    ...
    def visitForstmt(self, ctx:BKITParser.ForstmtContext):
        statements = list(map(lambda x: x.accept(self), ctx.statement()))
        return For(Id(ctx.ID().getText()),ctx.expr(0).accept(self),ctx.expr(1).accept(self),ctx.expr(2).accept(self),(reduce(lambda a,b: a + b, filter(lambda x: isinstance(x, list), statements), []),filter(lambda x: not(isinstance(x,list)), statements)))


    def visitWhilestmt(self, ctx:BKITParser.WhilestmtContext):
        statements = list(map(lambda x: x.accept(self), ctx.statement()))
        return While(ctx.expr().accept(self),(reduce(lambda a,b: a + b, filter(lambda x: isinstance(x, list), statements), []),filter(lambda x: not(isinstance(x,list)), statements)))


    def visitDowhilestmt(self, ctx:BKITParser.DowhilestmtContext):
        statements = list(map(lambda x: x.accept(self), ctx.statement()))
        return Dowhile((reduce(lambda a,b: a + b, filter(lambda x: isinstance(x, list), statements), []),filter(lambda x: not(isinstance(x,list)), statements)),ctx.expr().accept(self))
