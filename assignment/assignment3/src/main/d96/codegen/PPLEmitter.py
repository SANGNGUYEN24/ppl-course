...
class Emitter():
    ...
    *   @param isFinal true in case of constant; false otherwise
    '''
    def emitATTRIBUTE(self, lexeme, in_, isFinal, isStatic: boolean = False):
        #lexeme: String
        #in_: Type
        #isFinal: Boolean
        #value: String
    ...
    *   @param in the code to be printed out
    ...
    def emitInitNewStaticArray(self, name, size, eleType, frame):
        ...
        if(frame):
            frame.pop()
        if type(eleType) is StringType or ArrayType:
        ...
        result.append(self.jvm.emitPUTSTATIC(name, self.getJVMType(ArrayType(0, eleType))))
        return ''.join(result)
    ...
    def emitInitNewLocalArray1(self, name, size, eleType, frame):
        result = []
        result.append(self.emitPUSHICONST(size, frame))
        if(frame):
            frame.pop()
        if type(eleType) is StringType or ArrayType:
            ...
            result.append(self.jvm.emitNEWARRAY(self.getFullType(eleType)))
        result.append(self.jvm.emitPUTFIELD(name,self.getJVMType(ArrayType(0, eleType)) ))
        return ''.join(result)
    def emitInitNewLocalArray1(self, addressIndex, size, eleType, frame):
        result = []
        result.append(self.emitPUSHICONST(size, frame))
        if(frame):
            frame.pop()
